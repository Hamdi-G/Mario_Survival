...
io.sockets.on('connection', function (socket) {
	...
	// when the client emits 'sendchat', this listens and executes
	socket.on('sendpos', function (username_, x, y) {
		// we tell the client to execute 'updatepos' with 2 parameters
		//console.log(username_ + ","+ x +","+y);
		socket.broadcast.emit('updatepos', socket.username, username_, x, y);
	});
	...
	socket.on('adduser', function(username){
		...
		//var player = {'x':0, 'y':0, 'v':0, 'w':50, 'h':50 , 'score': 0}
    var player = new Player.Player(10, 10, 50, 50, 100);
		listOfPlayers[username] = player;

		io.sockets.emit('updatePlayers',listOfPlayers);
    /*io.sockets.emit('updateSmileys',listofSmileys);
    io.sockets.emit('updateSads',listOfSads);*/
	});
